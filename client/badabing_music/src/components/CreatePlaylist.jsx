...
import { createPlaylist, fetchPlaylists } from '../store/slices/playlistsSlice';
...
export const CreatePlaylist = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (createPlaylist.fulfilled.match(result)) {
        ...
        setError(result.payload || 'Failed to create playlist');
      }
    } catch (err) {
  ...
  const styles = {
    ...
    navBrand: { fontSize: '20px', fontWeight: 'bold', cursor: 'pointer', transition: 'opacity 0.3s' },
    backButton: {
      padding: '8px 16px',
    ...
    content: { maxWidth: '600px', margin: '60px auto', padding: '0 20px' },
    card: { backgroundColor: '#282828', borderRadius: '12px', padding: '40px', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' },
    header: { textAlign: 'center', marginBottom: '40px' },
    title: { fontSize: '36px', fontWeight: 'bold', marginBottom: '10px' },
    subtitle: { fontSize: '14px', opacity: 0.7, marginBottom: 0 },
    ...
    inputFocused: { borderColor: '#1db954', backgroundColor: '#252525', boxShadow: '0 0 0 3px rgba(29,185,84,0.1)' },
    textarea: {
      width: '100%',
      padding: '14px 16px',
      backgroundColor: '#1e1e1e',
    ...
    submitButton: { backgroundColor: '#1db954', color: '#fff' },
    cancelButton: { backgroundColor: 'transparent', color: '#fff', border: '2px solid #404040' },
    ...
    charCount: { fontSize: '12px', opacity: 0.6, marginTop: '6px', textAlign: 'right' },
  };

  return (
    <div style={styles.container}>
      <style>{`
        input:hover { border-color: #1db954 !important; }
        textarea:hover { border-color: #1db954 !important; }
